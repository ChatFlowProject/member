...
public class WebSecurity {
    ...
    protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        // 매니저에 등록할 MemberService는 UserDetailsService를 상속받은 클래스여야 함
        // MemberService의 구현체에는 loadUserByUsername(UserDetailsService의 추상메서드) 함수를 오버라이딩하여 유저 객체를 불러올 수 있음
...
//                                .access(new WebExpressionAuthorizationManager("hasIpAddress('::1')")) // '::1'은 IPv6의 루프백 주소로, IPv4의 127.0.0.1과 동일
    ...
    private AuthenticationFilter getAuthenticationFilter(AuthenticationManager authenticationManager) throws Exception {
        return new AuthenticationFilter(authenticationManager, memberService, env); // 미리 구현해 둔 AuthenticationFilter 인스턴스 생성
    }
}