...
import chatflow.memberservice.infrastructure.messaging.event.member.SignUpEvent;
...
public class SignServiceV2 {
    ...
    public SignUpResponse registerMember(SignUpRequest request) {
        ...
        eventPublisher.publishEvent(new SignUpEvent(member.getId().toString(), MemberPayload.from(member)));
